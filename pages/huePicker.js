...
const HuePicker = ({isOpen, elementId, setHue }) => {
  ...
  const getColorFromPosition = (sliderValue) => {
    ...
    if (position === 0) {
      ...
      setHue(
        `rgb(${255 - 255 / (proportion / (position - proportion))},255,0)`
      );
    } else if (position >= proportion * 2 && position < proportion * 3) {
      setHue(
        `rgb(0, 255, ${255 / (proportion / (position - proportion * 2))})`
      );
    } else if (position >= proportion * 3 && position < proportion * 4) {
      setHue(
        `rgb(0, ${255 - 255 / (proportion / (position - proportion * 3))}, 255)`
      );
    } else if (position >= proportion * 4 && position < proportion * 5) {
      setHue(
        `rgb(${255 / (proportion / (position - proportion * 4))}, 0, 255)`
      );
    } else if (position >= proportion * 4 && position < proportion * 5) {
      setHue(
        `rgb(${255 / (proportion / (position - proportion * 4))}, 0, 255)`
      ...
      setHue(
        `rgb(255, 0, ${255 - 255 / (proportion / (position - proportion * 5))})`
      );
    } else if (position === proportion * 6) {
      setHue(`rgb(255,0,0)`);
    }
  };
  ...
  const listener = useCallback((e) => {
    const huePicker = document.getElementById(id);
    let valueY = e.clientY - huePicker.getBoundingClientRect().y;
  ...
  return (
    ...
    >
      <div
        className="cp cp-dropdown-hue-draggable"
        style={{
          top: sliderY - 2,
        }}
      ></div>
